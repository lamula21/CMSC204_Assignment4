...
public class CourseDBStructureTest {
	...
	public void testHashTable()
	{
		...
		assertEquals(20, cds.hashTableOfCourses.length);
		CourseDBElement cde = new CourseDBElement("CMSC500", 39999, 4, "SC100", "Nobody InParticular"); 
		cds.add(cde);
		LinkedList<CourseDBElement> list = cds.hashTableOfCourses[cde.hashCode()%cds.getTableSize()];
		assertEquals(39999, list.get(0).getCRN());
		
		cds = new CourseDBStructure("Testing",20);
		assertEquals(20, cds.hashTableOfCourses.length);	
		cds.add(cde);
		list = cds.hashTableOfCourses[cde.hashCode()%20];
		assertEquals(39999, list.get(0).getCRN());
	}
}
